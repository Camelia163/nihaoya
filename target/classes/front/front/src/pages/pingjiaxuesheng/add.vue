<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="180px"
			>
			<el-form-item class="add-item" label="评价编号" prop="pingjiabianhao">
			...
			<el-form-item class="add-item"  label="评级" prop="pingji">
				<el-select v-model="ruleForm.pingji" placeholder="请选择评级" :disabled=" false  ||ro.pingji" >
					<el-option
			...
			<el-form-item class="add-item" label="工作态度" prop="gongzuotaidu">
				<el-input v-model="ruleForm.gongzuotaidu" 
					placeholder="工作态度" clearable :disabled=" false  ||ro.gongzuotaidu"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="工作质量" prop="gongzuozhiliang">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="工作质量"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`pingjiaxuesheng/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`pingjiaxuesheng/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
