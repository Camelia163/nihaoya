	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('gangweixinxi','新增')" type="primary" @click="add('/index/gangweixinxiAdd')">
				...
				<div class="list3 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						...
						<div class="item-info">
							...
							<div class="desc ql-snow ql-editor" v-html="item.gangweiyaoqiu"></div>
						</div>
					</div>
				</div>
			</div>
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/zhuanye/zhuanye').then(res => {
				if (res.data.code == 0) {
					this.zhuanyeOptions = res.data.data;
					this.selectOptionsList.push({name:'专业',list:this.zhuanyeOptions,tableName: 'zhuanye',check: -1})
				}
			});
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.zhuanye != '') searchWhere.zhuanye = this.formSearch.zhuanye;
				if(!this.centerType){
					params['sfsh'] = '是';
				}
				...
				this.$http.get(`gangweixinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/gangweixinxiDetail', query: params});
			},
