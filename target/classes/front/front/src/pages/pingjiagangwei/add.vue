<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="180px"
			>
			<el-form-item class="add-item" label="反馈编号" prop="fankuibianhao">
				<el-input v-model="ruleForm.fankuibianhao" placeholder="反馈编号" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="单位账号" prop="danweizhanghao">
				<el-select  @change="danweizhanghaoChange" v-model="ruleForm.danweizhanghao" placeholder="请选择单位账号">
					<el-option
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/yonggongdanwei/danweimingcheng', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.danweizhanghaoOptions = res.data.data;
					}
				});
			...
			danweizhanghaoChange () {
				this.$http.get('follow/yonggongdanwei/danweimingcheng?columnValue=' + this.ruleForm.danweizhanghao, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						if(res.data.data.danweimingcheng){
							this.ruleForm.danweimingcheng = res.data.data.danweimingcheng
						}
					}
				});
			},
			...
			// 多级联动参数
			info() {
				this.$http.get(`pingjiagangwei/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`pingjiagangwei/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
