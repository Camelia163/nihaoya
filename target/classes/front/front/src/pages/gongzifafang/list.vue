	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('gongzifafang','新增')" type="primary" @click="add('/index/gongzifafangAdd')">
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/gangweixinxi/gangweimingcheng').then(res => {
				if (res.data.code == 0) {
					this.gangweimingchengOptions = res.data.data;
					this.selectOptionsList.push({name:'岗位名称',list:this.gangweimingchengOptions,tableName: 'gangweimingcheng',check: -1})
				}
			});
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`gongzifafang/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/gongzifafangDetail', query: params});
			},
