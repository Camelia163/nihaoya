	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/gongzifafang?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">时薪</div>
						<div class="text "  >{{detail.shixin}}</div>
					</div>
					<div class="item">
						<div class="lable">时长/小时</div>
						<div class="text "  >{{detail.shizhang}}</div>
					</div>
					<div class="item">
						<div class="lable">总工资</div>
						<div class="text "  >{{detail.zonggongzi}}</div>
					</div>
					<div class="item">
						<div class="lable">录入时间</div>
						<div class="text "  >{{detail.lurushijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('gongzifafang','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('gongzifafang','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('gongzifafang','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="payBtn" v-if="(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('gongzifafang','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			payClick(){
				...
				localStorage.setItem('paytable','gongzifafang')
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/gongzifafang', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/gongzifafangAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此工资发放？') .then(_ => {
					this.$http.post('gongzifafang/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
